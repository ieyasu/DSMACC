...
* for wmo values
      ...
      INTEGER ierr
      REAL yg(kw), yg2(kw), o3xs(kw)

* for Molina & Molina values

      INTEGER n1, n2, n3
      REAL x2(kdata), y2(kdata), x3(kdata), y3(kdata)
      REAL s226(kw), s263(kw), s298(kw), s273(kw), s203(kw)
...
* use value at 273 K
      ...
      n2 = 158
      DO i = 1, n1
         READ(kin,*) idum, a1, a2, dum, dum, dum, y2(i), y1(i)
         x1(i) = (a1+a2)/2.
         x2(i) = x1(i)
      ENDDO
      CLOSE (kin)
...
* assign:
      ...
      DO 10 iw = 1, nw-1
         DO 20 iz = 1, nz
            ...
            IF ( wl(iw) .GE. 240.5  .AND. wl(iw+1) .LE. 350. ) THEN
     ...
     $                 (s298(iw)-s263(iw))*(tlev(iz)-263.)/(298.-263.)
            ...
            ELSE
                  xs(iz,iw) = s203(iw) + 
     $                 (s273(iw)-s203(iw))*(tlev(iz)-203.)/(273.-203.) 
